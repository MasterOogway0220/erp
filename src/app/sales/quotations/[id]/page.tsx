              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                {quotation.revisions && quotation.revisions.length > 1 && (
                  <div className="flex items-center gap-1 ml-4 print:hidden">
                    ...
                    <span className="text-[10px] uppercase font-bold text-muted-foreground mr-1">All Versions:</span>
                    <div className="flex gap-1">
                      {quotation.revisions.map((rev: any) => (
                        <Link
                          key={rev.id}
                          href={`/sales/quotations/${rev.id}`}
                          className={`text-[10px] px-1.5 py-0.5 rounded border transition-colors ${rev.id === quotation.id
          ...
          <div className="flex gap-2 print:hidden">
            {quotation.status !== "draft" && (
              <Button variant="outline" asChild>
                <Link href={`/sales/quotations/new?parentId=${quotation.id}`}>
