...
export async function POST(request: NextRequest) {
  ...
  for (const item of items) {
    dispatchedByProduct[item.product_id] = (dispatchedByProduct[item.product_id] || 0) + item.quantity
  }
  
  for (const soItem of so.items) {
    const dispatchedQty = dispatchedByProduct[soItem.product_id] || 0
    if (dispatchedQty > 0) {
      const newDeliveredQty = soItem.delivered_quantity + dispatchedQty
      await adminClient
