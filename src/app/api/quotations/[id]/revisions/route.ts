...
export async function POST(
  ...
  const nextVersionNumber = (parent.version_number || 0) + 1
  ...
  // Create version snapshot (for historical view/comparison)
  await adminClient.from('quotation_versions').insert({
    quotation_id: revision.id,
    version_number: nextVersionNumber,
    version_label: `Rev.${String(nextVersionNumber).padStart(2, '0')}`,
